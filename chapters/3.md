...
Any device connected to the internet is assigned an IP address which is an identifier to help route traffic to the correct user. However, this IP address can easily be linked to a user's physical location and identity.

The ability to match Monero activity with transactions to IP addresses has several significant downsides. Some of the cryptographic measures described in the previous section to protect Monero users may be partially circumvented if the IP addresses from node connection logs are analyzed to identify Monero users.

It is worth considering the unfortunate scenarios that may arise when Monero network activity is matched with physical location and identity.
