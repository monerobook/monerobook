...
2. The sender also demonstrates that the inputs balance the outputs, which is non-trivial given that ring signatures contain decoys to prevent the verifying party from knowing the true source of the input funds! Homomorphic Pedersen commitments enable the sender to prove that one of the potential inputs has a zero difference with the outputs, without revealing the amount in the process.
...
To prevent excessive blocksize growth, Monero mining protocol includes a penalty function that decreases the coinbase reward for oversize blocks. The original CryptoNote authors included this consensus rule to limit the rate of block size expansion and avoid rapid blockchain bloat.
