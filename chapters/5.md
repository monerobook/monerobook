...
This feature is especially valuable for charities, which can share their view key to ensure transparency and accountability around donations. If you make a donation to a public address, you can use the view key to verify that the charity received your funds.
...
The network byte is used to differentiate between various cryptocurrencies and networks. CryptoNote coins specify the appropriate values in the file src/CryptoNote_config.h, for example
...
Lastly, the network byte, keys, and checksum are concatenated according to the Monero specification:
...
If a block is mined with size (B) that is larger the median size of the last 100 blocks (MN), part of the base reward is withheld, according to:
