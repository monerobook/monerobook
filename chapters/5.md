...
To ascertain whether a given output X (with public transaction key R) was sent to the primary address, the wallet checks calculations based on its public view key and public spend key. If the equality X==Hs(pV0* R)G+PS0 is true, then that output can be unlocked and spent!
