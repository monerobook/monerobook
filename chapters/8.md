...
![GUI Languages](https://raw.githubusercontent.com/monerobook/monerobook/master/resources/img/languages_gui.png)
...
Proof of payment verification is available through the GUI under “prove/check.” The screenshoot above shows the transaction ID, address, and transaction key from the Maria & Khan example in chapter 2.
...
Use the transfer command to send Monero. To send moneroj to a single address, you do not have to specify the mixin number (during the October 2018, a hard fork requires to have a fixed mixin number of 11); you have to text the recipient address, and the amount to send. To use the CLI to send the 0.06 XMR transaction described above, the command would be: 
...
If you are looking for a payment that has a known payment ID, you can simply specify this in the payments command, e.g.
...
As reported in the paragraph 5.3.2.2, a view-only wallet is a special wallet which contains the private view key in order to see a sum of all transactions received and sent. This can be helpful and useful for people who want to add transparency by permitting to others to see partially their transactions.
...
8. Enter your preferred Restore height or creation date. Hit next button.
