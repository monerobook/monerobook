...
In fact, they may not hold that much, since most banks operate on “fractional reserve” - meaning that their actual assets are allowed to be significantly less than the total balance promised to account owners.
...
* Negligence, when the bank makes a mistake,
* Financial issues, when the bank overextends their assets or goes out of business,
* Overregulation, where the bank denies access to your funds
* Hidden fees, where sending, withdrawing, transferring or converting your assets come with large fees

However, in many third world countries, lack of regulation and protection can cost individuals trusting banks their assets in many other ways:

* Malice and corruption, when the bank or a rogue employee steals your money,
* Hostile third parties, when the bank is robbed or a hacker thieves electronic funds.

Thankfully, the emerging technology behind the blockchain is capable of mitigating all of the above risks by creating decentralized databases that all parties can equally use, view, and verify.
...
Take a moment to consider the sensitive information that you generate each day. Who might you be comfortable knowing your credit card transactions? Reading your texts? Your search history, products you purchase, or even the ones you view; social media sites that you interact with, and the pornographic sites you browse on Incognito. Almost all of it is recorded, profiled, and monetized by your banks, payment processors, ISPs and giant tech companies. While some of this information is illegal to buy and sell today, you can never know about tomorrow. What country is it in? Who can leak it?
...
To avoid these dangers, Monero uses powerful cryptographic techniques to create a network that allows parties to interact without revealing the sender, recipient, or transaction amounts. To ensure the absolute security and anonymity of the ledger, Monero obfuscates each of these pieces of information in entirely different ways (which we will explore with more detail in Chapter X, "How Monero Works"). Like other cryptocurrencies, Monero has a decentralized ledger that all participants can download and verify for themselves.

From a users standpoint, one of Monero’s crucial defining features is enforced privacy by default. Users are entirely prevented from initializing transactions that are potentially insecure; ensuring users with little or no technical ability are guarenteed to broadcast non-revealing transactions, obfuscated by layers of various anonymity techniques. If a user prioritizes transparency, they can selectively reveal information off-chain, rather than committing to a completely public record.
...
Despite not being able to see the the account balances or transaction amount, miners can continue to verify transactions haven't been cheated in the network with some really neat cryptography. Using some interesting proofs we explain later, miners can verify no Monero has been created or stolen from an unauthorized address, and no Monero has been double-spent. In each of these transactions they validate, nobody including the miners themselves can view the source, amount or destination.
