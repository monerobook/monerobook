...
Monero uses the CMake build system and a top-level makefile that invokes cmake commands, as needed. Once you've installed the dependencies, change to the root of the source code directory and execute the make command to begin the build. The process may take up to an hour or two. Once the code has finished building, you can find the Monero binaries in the build folder. 
